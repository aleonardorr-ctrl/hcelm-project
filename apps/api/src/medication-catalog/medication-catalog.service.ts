...
export class MedicationCatalogService {
  ...
  async listCatalog(params: {
    ...
    const pageSize = Math.min(100, Math.max(10, Math.trunc(params.pageSize || 50)));
    ...
    if (params.productType?.trim()) where.productType = params.productType.trim().toUpperCase();
  ...
  async generateTemplate() {
    ...
    const productColumns: Array<[string, number]> = [
      ...
      ['titular_registro', 28], ['registro_sanitario', 22], ['codigo_atc', 16],
      ['codigo_pnum', 18], ['requiere_receta', 18], ['controlado', 14],
      ['cadena_frio', 14], ['afecto_igv', 14], ['activo', 12], ['observaciones', 38],
    ];
    ...
    instructions.addRows([
      ...
      ['Gerencia', '', 'Podra consolidar stock, valorizacion, margen y proximos vencimientos.'],
    ]);
    ...
    return { fileName: 'plantilla_maestro_corporativo_farmacia.xlsx', buffer: Buffer.from(buffer) };
  }

  async previewImport(params: { tenantId: string; userId: string | null; file: Express.Multer.File }) {
    const workbook = new ExcelJS.Workbook();
    try {
    ...
    if (Math.max(0, sheet.actualRowCount - 1) > MAX_PRODUCTS) {
      throw new BadRequestException(`El archivo supera ${MAX_PRODUCTS} productos.`);
    }
    ...
    sheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      ...
      if (errors.length) {
        invalidRows.push({ rowNumber, code: internalCode || null, name: genericName || null, errors });
        return;
      }
      ...
      try {
        ...
        invalidRows.push({ rowNumber, code: internalCode, name: genericName, errors: [error.message] });
        return;
      }
      parsed.push({
    ...
    const existingByCode = new Map(existing.map((item) => [item.internalCode, item]));
    ...
    const report = { summary, validRows: productRows, invalidRows, inventoryRows: lotResult.rows, invalidInventoryRows: lotResult.invalidRows };
    const record = await this.prisma.catalogImport.create({
      data: {
        tenantId: params.tenantId, catalogType: CATALOG_TYPE,
        sourceFileName: params.file.originalname, source: 'Excel', status: 'PREVIEWED',
        totalRows: summary.totalRows, validRows: summary.validRows,
        invalidRows: summary.invalidRows, errorRows: summary.invalidRows,
        validationReport: report as any, createdById: params.userId,
      },
    });
    return {
      previewId: record.id,
  ...
  async applyImport(params: { tenantId: string; userId: string | null; previewId: string }) {
    const record = await this.prisma.catalogImport.findFirst({
      where: { id: params.previewId, tenantId: params.tenantId, catalogType: CATALOG_TYPE },
    });
    if (!record) throw new NotFoundException('Previsualizacion no encontrada.');
    if (record.status !== 'PREVIEWED') throw new ConflictException('La importacion no esta disponible.');
    const report: any = record.validationReport || {};
    if (report.invalidRows?.length || report.invalidInventoryRows?.length) {
      throw new BadRequestException('Corrija los errores antes de confirmar la importacion.');
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      const claimed = await tx.catalogImport.updateMany({
        where: { id: record.id, status: 'PREVIEWED' }, data: { status: 'APPLYING' },
      });
      if (claimed.count !== 1) throw new ConflictException('La importacion ya fue procesada.');
      for (const row of rows) {
        const data = this.productData(row, record.sourceFileName, params.userId);
        if (row.action === 'CREATE') await tx.medication.create({ data: { tenantId: params.tenantId, ...data } });
        if (row.action === 'UPDATE') {
          await tx.medication.update({
            where: { tenantId_internalCode: { tenantId: params.tenantId, internalCode: row.internalCode } },
            data,
          });
        }
      }
      const products = await tx.medication.findMany({
        where: { tenantId: params.tenantId, internalCode: { in: [...new Set(lots.map((lot) => lot.internalCode))] } },
        select: { id: true, internalCode: true },
      });
      const byCode = new Map(products.map((item) => [item.internalCode, item.id]));
      for (const lot of lots) {
        const medicationId = byCode.get(lot.internalCode);
        if (!medicationId) continue;
        await tx.medicationInventoryLot.upsert({
          where: { tenantId_medicationId_businessUnit_warehouse_lotNumber: {
            tenantId: params.tenantId, medicationId, businessUnit: lot.businessUnit,
            warehouse: lot.warehouse, lotNumber: lot.lotNumber,
          } },
          update: this.lotData(lot, record.sourceFileName),
          create: { tenantId: params.tenantId, medicationId, createdById: params.userId,
            ...this.lotData(lot, record.sourceFileName) },
        });
      }
      await tx.catalogImport.update({
        where: { id: record.id },
        data: { status: 'COMPLETED', createdRows, updatedRows, skippedRows, completedAt: new Date() },
      });
    }, { timeout: 180_000 });
  ...
  private parseLots(workbook: ExcelJS.Workbook, productCodes: Set<string>) {
    ...
    sheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      ...
      const errors: string[] = [];
      if (!productCodes.has(internalCode)) errors.push('El producto no existe en la hoja Productos.');
      if (!['FARMACIA', 'DROGUERIA', 'CONSULTORIO'].includes(businessUnit)) errors.push('Unidad de negocio no valida.');
      const currency = this.code(value('moneda')) || 'PEN';
      if (!['PEN', 'USD'].includes(currency)) errors.push('Moneda no valida. Use PEN o USD.');
      const identity = `${internalCode}|${businessUnit}|${warehouse}|${lotNumber}`;
      if (identities.has(identity)) errors.push('Lote duplicado.');
      let expirationDate: string | null = null;
      try { expirationDate = this.date(value('fecha_vencimiento')); } catch (error: any) { errors.push(error.message); }
      let stock = 0;
      let minimumStock = 0;
      ...
      if (stock < 0 || minimumStock < 0) errors.push('Stock y stock minimo no pueden ser negativos.');
  ...
  private productData(row: ProductRow, source: string, createdById: string | null) {
    return {
      ...
      laboratory: row.laboratory, manufacturer: row.manufacturer,
      registrationHolder: row.registrationHolder, sanitaryRegistration: row.sanitaryRegistration,
      atcCode: row.atcCode, pnumCode: row.pnumCode,
      requiresPrescription: row.requiresPrescription, controlled: row.controlled,
      coldChain: row.coldChain, taxable: row.taxable, active: row.active,
      observations: row.observations, searchText: row.searchText, source, createdById,
    };
  }
  ...
  private lotData(row: LotRow, source: string) {
    return {
      ...
      currency: row.currency, supplier: row.supplier, active: row.active, source,
    };
  }

  private sameProduct(current: any, row: Omit<ProductRow, 'action'>) {
    const data = this.productData(row as ProductRow, current.source || '', current.createdById || null);
    return Object.entries(data).every(([key, value]) =>
      ['source', 'createdById'].includes(key) || String(current[key] ?? '') === String(value ?? ''),
    );
  }
  ...
  private headers(sheet: ExcelJS.Worksheet) {
    ...
    sheet.getRow(1).eachCell((cell, column) => result.set(this.header(this.text(cell.value)), column));
    return result;
  }
  ...
  private code(value: string) { return value.trim().toUpperCase().replace(/\s+/g, '_'); }
  private nullable(value: string) { const text = value.trim(); return text || null; }
  ...
  private boolean(value: string, defaultValue: boolean) {
    ...
    throw new BadRequestException(`Valor booleano no valido: ${value}. Use SI o NO.`);
  }

  private number(value: string, defaultValue: number) {
    if (!value.trim()) return defaultValue;
    const result = Number(value.replace(',', '.'));
    if (!Number.isFinite(result)) throw new BadRequestException(`Valor numerico no valido: ${value}.`);
    return result;
  }

  private nullableNumber(value: string) { return value.trim() ? this.number(value, 0) : null; }

  private date(value: string) {
    const text = value.trim();
    if (!text) return null;
    const date = new Date(text.length === 10 ? `${text}T00:00:00` : text);
    if (Number.isNaN(date.getTime())) throw new Error(`Fecha no valida: ${value}.`);
