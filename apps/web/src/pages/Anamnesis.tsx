        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            ...
            <h2 className="text-xl font-extrabold text-blue-900">
              N.° HCE Digital: {currentHceNumber}
            </h2>
            <p className="mt-1 text-sm text-blue-900">
              Paciente:{" "}
              <span className="font-semibold">
                {currentPatient?.fullName || "Paciente no seleccionado"}
              </span>
            </p>
          </div>
          ...
          <div className="rounded-lg bg-white px-4 py-3 text-sm text-slate-700 border border-blue-100">
            ...
            <p>
              ...
              {formData.encounterId || encounterIdFromUrl || "—"}
            </p>
          </div>
        </div>
      </div>
            ...
            {medResults.length > 0 && (
              <div className="border rounded bg-white mb-4">
                {medResults.map((m) => (
                  ...
                  >
                    <b>{m.genericName}</b>
                    {m.commercialName ? ` (${m.commercialName})` : ""} -{" "}
                    {m.concentration} - {m.presentation} - {m.route}
                  </button>
                ))}
              </div>
            )}
