...
function formatHceNumber(patient?: Patient | null): string {
  const value = patient?.hceNumber;
  return typeof value === "string" && value.trim()
    ? value.trim()
    : "HCE pendiente de generar";
}

function hasPendingHce(patient?: Patient | null): boolean {
  return !patient?.hceNumber || !patient.hceNumber.trim();
}
...
function normalizePatient(patient: any): Patient {
  ...
  return {
    ...
    hceNumber:
      patient?.hceNumber || patientData?.hceNumber || null,
    firstName:
...
export default function Patients() {
  ...
  function handleEditPatient(patient: Patient) {
    ...
    if (!patientId) {
      setError("No se puede editar este paciente porque no tiene un ID técnico válido.");
      return;
    }
    ...
    const splitName = splitFullName(normalizedPatient.fullName || normalizedPatient.name);

    setForm({
      documentType: normalizedPatient.documentType || "DNI",
      documentNumber: normalizedPatient.documentNumber || "",
      firstName: normalizedPatient.firstName || splitName.firstName || "",
      paternalLastName:
        normalizedPatient.paternalLastName || splitName.paternalLastName || "",
      maternalLastName:
        normalizedPatient.maternalLastName || splitName.maternalLastName || "",
      sex: normalizedPatient.gender || normalizedPatient.sex || "",
      birthDate: normalizeDateForInput(normalizedPatient.birthDate),
      phone: normalizedPatient.phone || "",
      email: normalizedPatient.email || "",
      address: normalizedPatient.address || "",
      allergies: normalizedPatient.allergies || "",
      ...
      <div className="p-6 bg-white rounded shadow border border-gray-200">
        ...
        {selectedPatient && (
          <div className="mt-4 p-4 border border-blue-200 bg-blue-50 rounded">
            ...
            <div className={`mt-3 inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-sm font-bold ${hasPendingHce(selectedPatient) ? "border-amber-300 bg-amber-50 text-amber-800" : "border-blue-300 bg-white text-blue-800"}`}>
              <span>N.° HCE Digital:</span>
              <span>{formatHceNumber(selectedPatient)}</span>
            </div>
                ...
                {filteredPatients.map((patient) => (
                  ...
                  >
                    <td className="border px-3 py-2">
                      <span
                        className={`inline-flex rounded-full px-2 py-1 text-xs font-bold ${hasPendingHce(patient) ? "bg-amber-100 text-amber-800" : "bg-blue-100 text-blue-800"}`}
                      >
